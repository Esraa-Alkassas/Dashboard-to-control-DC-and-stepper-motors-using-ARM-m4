...
// this commented code if i will set the whole port but the uncommented if i will set the first half of the port, regardless of the function name...
...
void MGPIO_voidSetSecHalfValuePort (MGPIOx_t Copy_uddtPortNum, u8 Copy_u8PortValue)
{
	switch(Copy_uddtPortNum)
	{
	case MGPIO_PORTA:
		Clr_BITS(MGPIOA->ODR,65280);
		//Clr_ALL_BITS(MGPIOA->ODR);
		MGPIOA->ODR |= (u32)(Copy_u8PortValue<<8);
		break;

	case MGPIO_PORTB:
		//Clr_ALL_BITS(MGPIOB->ODR);
		Clr_BITS(MGPIOB->ODR,65280);
		MGPIOB->ODR |= (u32)(Copy_u8PortValue<<8);
		break;

	case MGPIO_PORTC:
		//Clr_ALL_BITS(MGPIOC->ODR);
		Clr_BITS(MGPIOC->ODR,65280);
		MGPIOC->ODR |= (u32)(Copy_u8PortValue<<8);
		break;
